...
def create_tables():
    ...
    # Create Submissions table (for auto grading trigger example)
    cursor.execute('''
...
def insert_sample_data():
    ...
    try:
        ...
        print("Math exam already exists")
    ...
    try:
        cursor.execute("INSERT INTO questions (exam_id, question_text, difficulty_level, correct_answer, marks) VALUES (1, 'What is 2+2?', 'easy', '4', 5)")
    except sqlite3.IntegrityError:
        print("Question 1 for Math Exam already exists")

    try:
        cursor.execute("INSERT INTO questions (exam_id, question_text, difficulty_level, correct_answer, marks) VALUES (1, 'What is 3+5?', 'easy', '8', 5)")
    except sqlite3.IntegrityError:
        print("Question 2 for Math Exam already exists")

    try:
        cursor.execute("INSERT INTO questions (exam_id, question_text, difficulty_level, correct_answer, marks) VALUES (2, 'What is H2O?', 'medium', 'Water', 10)")
    except sqlite3.IntegrityError:
        print("Question for Science Exam already exists")
    
    # Insert sample enrollments
    try:
        cursor.execute("INSERT INTO enrollments (student_id, exam_id) VALUES (3, 1)")
    except sqlite3.IntegrityError:
        print("Enrollment for Math Exam already exists")

    try:
        cursor.execute("INSERT INTO enrollments (student_id, exam_id) VALUES (3, 2)")
    except sqlite3.IntegrityError:
        print("Enrollment for Science Exam already exists")
    
    # Insert sample results
    try:
        cursor.execute("INSERT INTO results (enrollment_id, score) VALUES (1, 100)")
    except sqlite3.IntegrityError:
        print("Result for Math Exam already exists")

    try:
        cursor.execute("INSERT INTO results (enrollment_id, score) VALUES (2, 90)")
    except sqlite3.IntegrityError:
        print("Result for Science Exam already exists")
    
    connection.commit()
    connection.close()

# Function to create all necessary views and triggers
def setup_views_and_triggers():
    connection = get_db_connection()
    cursor = connection.cursor()

    # Create User Roles View
    cursor.execute('''
    CREATE VIEW IF NOT EXISTS user_roles AS
    SELECT user_id, username, role FROM users
    ''')

    # Create Upcoming Exams View
    cursor.execute('''
    ...
    # Create Student Performance History View
    cursor.execute('''
    ...
    # Create Exam Result Statistics View
    cursor.execute('''
    ...
    # Create Question Update Log Trigger
    cursor.execute('''
    ...
    # Create New Exam Notification Trigger
    cursor.execute('''
    ...
    # Create Exam Result Notification Trigger
    cursor.execute('''
    ...
    # Create Student Enrollment Notification Trigger
    cursor.execute('''
...
#Initial setup to create tables, insert sample data,and setup views/ or  triggers
def initial_setup():
    create_tables()
    insert_sample_data()
